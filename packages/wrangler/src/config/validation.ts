...
const validateConstellationBinding: ValidatorFn = (
	...
	let isValid = true;
	// D1 databases must have a binding and a project.
	if (!isRequiredProperty(value, "binding", "string")) {
		diagnostics.errors.push(
			`"${field}" bindings should have a string "binding" field but got ${JSON.stringify(
				value
	...
	if (!isRequiredProperty(value, "project", "string")) {
		diagnostics.errors.push(
			`"${field}" bindings must have a "project" field but got ${JSON.stringify(
				value
			)}.`
		);
		isValid = false;
